...
quality and channel volatility.

Endpoint vectors w_low and w_high are reproduced from the
predecessor MSc thesis (its Fig. 5.26) over the criteria order
(age, tabu, NNS):
    w_low  = (0.43, 0.52, 0.05)   # low mobility, beam-acquisition heavy
    w_high = (0.16, 0.36, 0.49)   # high mobility, NNS-tracking heavy
"""
...
class MCMD(Algorithm):
    ...
    def reset(self, state: BPLMState, context: dict) -> None:
        self._tabu: deque[tuple[int, int]] = deque(maxlen=self.tabu_tenure)
    ...
    def select_next_mbp(self, state, m, context):
        ...
        # Age: per-entry occasion count since last measurement.
        ages = state.age_matrix(current_m=m)
        # Normalize to [0, 1]
        age_norm = ages / (ages.max() + 1e-12)
        C_age = age_norm

        # NNS: 1 inside a small radius around OBP, fading to 0.
        kk, ll = np.indices((K, L))
        dist = np.maximum(np.abs(kk - ck), np.abs(ll - cl))
        C_nns = np.exp(-dist.astype(float) / self.nns_radius)

        # Tabu: 1 inside an annulus around OBP, with a penalty for tabu list.
        in_band = ((dist >= 1) & (dist <= self.nns_radius + 1)).astype(float)
        C_tabu = in_band.copy()
        for tk, tl in self._tabu:
            C_tabu[tk, tl] *= 0.1  # heavy down-weight, not strict ban (allows aspiration)
        ...
        # Aggregate priority matrix R = sum_i w_i C^(i)
