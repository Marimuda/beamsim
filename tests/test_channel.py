...
def test_laplacian_subray_shape():
    ...
    offsets = _laplacian_subray_offsets(
        rng, n_clusters=12, n_rays=20, spread_rad=math.radians(17.0)
    )
...
def test_laplacian_subray_distribution_mean_and_scale():
    ...
    # Draw many clusters
    offsets = _laplacian_subray_offsets(rng, n_clusters=5000, n_rays=20, spread_rad=spread_rad)
...
def test_always_12_clusters():
    ...
    for los_prob in (0.0, 1.0):
        ...
        assert len(real.scatterer_xy) == 12, f"Expected 12 clusters, got {len(real.scatterer_xy)}"
...
def test_geometric_cluster_power_closer_scatterer_higher_power():
    ...
    powers = _geometric_cluster_powers(bs_xy, ue_xy, scatterers, fc_hz, h_bs, h_ut, k_lin, False)
...
def test_self_blocker_attenuates_when_bs_at_back():
    ...
    params = ChannelParams(los_probability=1.0, k_factor_mean_db=30.0, k_factor_std_db=0.0)
...
def test_non_self_blocker_drifts_over_position():
    ...
    params = ChannelParams(los_probability=1.0, k_factor_mean_db=9.0, k_factor_std_db=0.0)
...
# ===========================================================================
# 15. disable_clusters flag — LOS-only variant (Fig 6.9 "without reflectors")
# ===========================================================================


def test_disable_clusters_equals_los_only():
    """With disable_clusters=True, channel_matrix must equal the LOS outer-product.

    The LOS outer product is:  h_los = los_amp * los_doppler * outer(a_ue, a_bs*)
    We verify this by checking that H with clusters disabled equals H with
    clusters enabled minus the cluster contribution.  The simpler proxy: run two
    realisations from the same seed — one with clusters, one without — and assert
    that the cluster-disabled version has strictly smaller Frobenius norm when the
    channel is LOS (cluster power adds energy).
    """
    ...
    # Build with clusters (default)
    rng_seed = 42
    params_with = ChannelParams(
        k_factor_mean_db=20.0,
        k_factor_std_db=0.0,
        ue_speed_mps=0.0,
        disable_clusters=False,
    )
    real_with = ChannelRealisation(
        params=params_with,
        bs_xy=bs_xy,
        bs_yaw=0.0,
        n_bs_elements=8,
    ...
    # Build without clusters (same seed for same geometry / scatterers)
    params_without = ChannelParams(
        k_factor_mean_db=20.0,
        k_factor_std_db=0.0,
        ue_speed_mps=0.0,
        disable_clusters=True,
    )
    real_without = ChannelRealisation(
    ...
    H_with = real_with.channel_matrix(ue_xy, ue_yaw)
    H_without = real_without.channel_matrix(ue_xy, ue_yaw)

    # disable_clusters=True must produce a different (cluster-free) matrix
    assert not np.allclose(H_with, H_without), (
        "disable_clusters=True should produce a different H than with clusters"
    )

    # With K=20 dB LOS dominates; disabling clusters should reduce total energy
    # only slightly (clusters carry 1/(1+K) ≈ 1% of power).
    norm_with = np.linalg.norm(H_with)
    norm_without = np.linalg.norm(H_without)
    # Both should be nonzero and finite
    assert norm_without > 0 and np.isfinite(norm_without)
    assert norm_with > 0 and np.isfinite(norm_with)
