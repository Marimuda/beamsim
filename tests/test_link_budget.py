...
def test_tx_amp_known_value():
    ...
    result = tx_amp_for_snr_db(target_db, distance_m, fc_hz, h_bs, h_ut, noise_amp, n_ue, n_bs)
...
def test_tx_amp_increases_with_target_snr():
    ...
    kwargs = dict(
        distance_m=100.0, fc_hz=28e9, h_bs=10.0, h_ut=1.5, noise_amp=1e-3, n_ue=4, n_bs=16
    )
...
def test_tx_amp_increases_with_distance():
    ...
    kwargs = dict(target_db=10.0, fc_hz=28e9, h_bs=10.0, h_ut=1.5, noise_amp=1e-3, n_ue=4, n_bs=16)
